...
class AdminBookGeneratorService {
    ...
    async generateBookPreview(bookType, generationData) {
        ...
        try {
            ...
            const totalPages = bookType === 'colorir' ? (pageCount * 2) + 2 : (pageCount * 2) + 2;
            ...
            (async () => {
                try {
                    ...
                    if (bookType === 'colorir') {
                        await this.generateColoringBookContent(book, bookVariation, characters, theme, parseInt(pageCount, 10), elementId, coverElementId);
                    } else if (bookType === 'historia') {
                        await this.generateStoryBookContent(book, bookVariation, characters, summary, parseInt(pageCount, 10), elementId, coverElementId);
                    }
    ...
    async generateColoringBookContent(book, bookVariation, characters, theme, innerPageCount, elementId, coverElementId) {
    ...
    async generateStoryBookContent(book, bookVariation, characters, summary, sceneCount, elementId, coverElementId) {
        ...
        for (const scene of storyPages) {
            // 1. Gera a página de TEXTO como imagem PRIMEIRO
            ...
            // 2. Gera a ILUSTRAÇÃO correspondente DEPOIS
